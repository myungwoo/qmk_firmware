...
- A 홀드가 켜진 상태에서 **물리 A를 눌렀다 떼는 경우**: 기본적으로 **A 홀드 해제(= A 뗀 것과 같게)**
  - 단, **물리 A를 누른 채 `A_TOG`를 누르면**(홀드 “확정”), 그 다음 **물리 A up은 무시**되어 **A 홀드가 유지**됩니다.
  - 즉: `A_TOG 탭 → 물리 A down → A_TOG 탭 → 물리 A up` 이어도 A는 계속 DOWN 입니다.
...
- `keymap.c`
  - `A_TOG (0x7E40)`, `AUTOCLICK (0x7E41)` 및 동작 로직 + `shutdown_user()` 가드
